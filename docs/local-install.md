...
本机菜单支持内置和自定义模板、已配置的打开方式、手动选择目录进行复制/移动，以及剪切后到目标目录粘贴。主应用用于设置，不再提供“文件操作台”。在“菜单管理”中可把常用操作放到一级菜单，其余收进 RightMouse 子菜单。剪切状态会在剪贴板改变或主应用重启后失效。“常用目录”已移除。点击“复制到…”或“移动到…”直接选择目标；取消选择不会执行操作。同名文件默认询问。旧收藏记录仅保留兼容，不再显示，也不会删除实际文件夹。没有要求辅助功能、完全磁盘访问或管理员权限。退出主应用后，下次 Finder 操作会再次唤醒主应用。

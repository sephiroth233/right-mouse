...
本机菜单支持内置和自定义模板、已配置的打开方式、常用目录的打开/复制/移动，以及剪切后到目标目录粘贴。主应用用于设置，不再提供“文件操作台”。在“菜单管理”中可把常用操作放到一级菜单，其余收进 RightMouse 子菜单。剪切状态会在剪贴板改变或主应用重启后失效。常用目录在本机版通过系统选择器保存普通书签；旧安全作用域书签如显示不可用，请重新选择一次。没有要求辅助功能、完全磁盘访问或管理员权限。退出主应用后，下次 Finder 操作会再次唤醒主应用。

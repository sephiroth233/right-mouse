...
当前源码提供五个 macOS Services，已发布的 0.2.1 安装包尚不包含。安装包含该功能的构建后，先正常打开应用一次，然后在 Finder 中右键选择「服务」：
...
在“通用 → 本机连接”点击“停用并移除服务”，退出 RightMouse，然后将应用移到废纸篓。设置及未解决的异常恢复现场保留；不自动删除用户文件。

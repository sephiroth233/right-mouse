...
struct RightMouseHostCheck {
    ...
    static func main() async {
        do {
            let count = try await run() + runFollowupChecks() + runConflictChecks() + runRecentDestinationHostChecks() + runOpenWithChecks() + runOpenWithHostChecks() + runDiagnosticHostChecks() + runRetentionChecks() + runSetupExerciseChecks() + runStorageSeparationHostChecks() + runStagingPresentationChecks() + runStagingHostChecks()

...
struct RightMouseHostCheck {
    ...
    static func main() async {
        do {
            let count = try await run() + runFollowupChecks() + runConflictChecks() + runRecentDestinationHostChecks() + runOpenWithChecks() + runOpenWithHostChecks() + runDiagnosticHostChecks() + runRetentionChecks() + runRetentionStartupChecks() + runSetupExerciseChecks() + runStorageSeparationHostChecks() + runStagingPresentationChecks() + runStagingHostChecks() + runSourceRecoveryHostChecks() + runClipboardHostChecks() + runLocalFinderHostChecks() + runAuthenticatedMenuHostChecks() + runSilentOperationChecks() + runApplicationLaunchChecks() + runFinderServicesChecks()
            print("PASS: \(count) host-side checks (HostController integration and open-with planning)")
        } catch {

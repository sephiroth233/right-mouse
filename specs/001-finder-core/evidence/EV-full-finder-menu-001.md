...
后续用户决定移除常用目录。已移除侧栏、菜单管理开关和 Finder 收藏入口；旧收藏及置顶 ID 不能恢复菜单。复制到/移动到改为直接选择目标，无额外子菜单；同名冲突仍询问。旧数据读取和底层历史命令保留兼容，不删除用户文件夹。主界面现在为五个设置页面。

本次 370 项核心、472 项宿主检查通过（`.build/remove-favorites-core.log`、`.build/no-favorites-host.log`）。核心首次受限执行遇到文件协调权限失败，正常本机权限运行同一二进制通过。新增回归验证旧收藏和置顶入口不可恢复，以及一级复制/移动标签与选择器动作。

从最终 DMG 安装副本实际查看五个侧栏入口、菜单管理列表及 Finder 右键菜单，均无常用目录。选择 RightMouse → 复制到… 后直接出现“请选择此操作的目标目录”系统选择框；点击取消返回设置页。

本次构建 `.build/no-favorites-preview`，证书指纹 `43f61ad1e21b18fdd8e759945cae249009547bae`，嵌套签名严格验证通过。DMG 已通过镜像及 SHA-256 校验；当前 `dist/RightMouse-0.2.0-local-arm64.dmg` 的 SHA-256 更新为 `afdfb16725f7196928bcd4a39073e59a25595cafb496bb664f50faec7ad09a5d`，替代上节历史产物。已从 DMG 更新并启动 `.build/dmg-installed/RightMouse.app`，保留正常用户配置。

分步提交 `c2d2ee8` 移除契约、`a983c12` 菜单与设置实现。

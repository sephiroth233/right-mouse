...
跨卷来源清理在原路径核验后，先把来源以禁止覆盖的重命名移入同父私有容器。journal 先保存 sourceCleanupURL、容器身份与 isolating 意图，再保存 isolated 观察结果；对隔离对象完整复核后才逐项删除。原路径上新出现的文件不属于清理范围。失败保留隔离位置并进入 needsReview，恢复页展示“待核对的来源副本”，不自动移回或删除。成功清理后清空路径和身份引用，状态为 completed；存在隔离引用或其他清理状态时，过期保留策略不得删除其关联证据。详细验证见[来源隔离记录](evidence/EV-source-isolation-001.md)。
